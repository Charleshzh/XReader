...
/** Estimate card height: cover (3:4, 1rem=16px → ~200px) + title + author + badge ~ 280px */
const GRID_CARD_ESTIMATE = 280;
...
export function BookshelfPage() {
  ...
  // Measure container width for responsive grid columns
  ...
  // Compute rows for grid mode
  const rowCount = viewMode === "grid" ? Math.ceil(filtered.length / gridCols) : filtered.length;

  // Estimate height based on view mode
  const estimateSize = viewMode === "grid" ? () => GRID_CARD_ESTIMATE : () => LIST_ROW_HEIGHT;
          ...
          {loading ? (
            ...
            virtualizer.getVirtualItems().map((virtualRow) => {
              if (viewMode === "grid") {
                ...
                const rowBooks = filtered.slice(startIdx, startIdx + gridCols);
                return (
                  <div
                    key={virtualRow.key}
                    style={{
