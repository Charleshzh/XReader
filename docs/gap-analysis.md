...
2. **book/db/sync 模块单元测试** — 8-16h

### 中期（1-2 周）

3. **Playwright E2E 冒烟测试** — 16-24h，需要 CI 配置 + 测试 fixture
4. **macOS/Linux 打包验证** — 8-16h，需要对应平台
5. **文本选区高亮渲染** — 16-24h，HTML 内容标记 + 定位算法
6. **书源规则测试工具（搜索预览）** — 4-8h

---

## 7. 总结

**核心 MVP + 高优先级缺口已交付**：书架导入（虚拟列表）、三种格式阅读、书签笔记（支持导出）、阅读统计（设置持久化）、Legado 规则引擎（含发现页）、WebDAV 双向增量同步、自动更新。

**近期关闭 (d70b1c3)**：虚拟列表、发现页 (ruleExplore)、双向云同步合并、书签/笔记 Markdown 导出。完成率 82% → 87%。

**剩余差距**：

- 测试覆盖严重不足（仅规则引擎有测试，~15%）
- macOS/Linux 打包未验证
- 文本选区高亮渲染未实现
- 平台兼容测试未完成
